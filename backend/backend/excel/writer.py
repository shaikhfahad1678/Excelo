...
def generate_excel_workbook(
    ...
    for sheet_name, transactions in sheet_data_map.items():
        ...
        # Dynamically discover all unique keys in the transactions to prevent column loss
        all_keys = []
        for tx in transactions:
            for k in tx.keys():
                if k not in all_keys and k not in ["Currency", "Sr No.", "Validation Status", "Validation Details", "Confidence", "diagnostics"]:
                    all_keys.append(k)

        # Standard header priority ordering
        columns_order = ["Sr No."]
        std_keys = ["Date", "Description", "Cheque No.", "Debit", "Credit", "Balance"]
        for k in std_keys:
            if k in all_keys:
        ...
        for k in all_keys:
            if k not in columns_order:
                columns_order.append(k)
        columns_order.append("Validation Status")
