...
def generate_sample():
    """
    Generates a sample bank statement for instant testing.
    """
    card = statement_service.register_sample_pdf()
    return {"status": "success", "file": card}
...
def validate_transactions_endpoint(req: ValidateRequest):
    ...
    validated_txs, summary = validate_transactions(req.transactions, tolerance=req.tolerance or 0.05)
...
def generate_excel_endpoint(req: GenerateExcelRequest):
    ...
    """
    try:
        export_filepath = statement_service.generate_export(req.file_ids, export_format=req.format or "xlsx")
...
def get_settings():
    ...
    GET /settings
    """
    return {"status": "success", "settings": statement_service.get_settings()}
...
def update_settings(settings: Dict[str, Any] = Body(...)):
    ...
    POST /settings
    """
    updated = statement_service.update_settings(settings)
