      ...
      <div className="max-w-[1700px] mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {processingCount > 0 ? (
            ...
            <div className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-lg bg-neutral-50 border border-neutral-200/60 text-neutral-600 text-[11px] font-medium">
              <span className={`w-2 h-2 rounded-full ${isBackendConnected ? 'bg-emerald-500' : 'bg-rose-500'}`} />
              <span>{isBackendConnected ? '6 Dedicated Bank Engines Ready' : 'Backend Disconnected'}</span>
            </div>
          )}
